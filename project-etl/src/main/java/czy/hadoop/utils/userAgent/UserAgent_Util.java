...
public class UserAgent_Util {
	 ...
	 * @return 返回具体的值
	 */
	public static UserAgentInfo_Model analyticUserAgent(String userAgent) {
		UserAgentInfo_Model result = null;
		if (!(userAgent == null || userAgent.trim().isEmpty())) {  // 此时userAgent不为null，而且不是由全部空格组成的
			try {
				UserAgentInfo info = null;
				info = uasParser.parse(userAgent);
				result = new UserAgentInfo_Model();
